...
class Mapster_Wordpress_Maps_Admin {
	...
	public function enqueue_scripts( $hook_suffix ) {
  	...
  	if( $current_screen->id === "mapster-wp-map" ) {
    	...
    	global $post;
			...
			wp_enqueue_script('mapster_map_turf_js', plugin_dir_url( __FILE__ ) . "js/vendor/custom-turf.js", array('mapster_map_map_library'), $this->version);
			...
			wp_register_script($this->plugin_name, plugin_dir_url( __FILE__ ) . '/js/dist/mwp.js', array('mapster_map_geosearch_js'), $this->version, true );
  ...
  function set_custom_mapster_map_column($columns) {
	...
	/**
	 * Add User Submissions button
	 *
	 * @since    1.0.0
	 */
	function mapster_add_user_submission() {
    global $current_screen;

    // Not our post type, exit earlier
    // You can remove this if condition if you don't have any specific post type to restrict to.
    if ('mapster-wp-location' == $current_screen->post_type || 'mapster-wp-line' == $current_screen->post_type || 'mapster-wp-polygon' == $current_screen->post_type) {
			add_thickbox();

    	?>
        <script type="text/javascript">
					jQuery(function () {
						jQuery('hr.wp-header-end').before("<a name='User Submissions' href='#TB_inline?width=600&inlineId=mapster-user-submission-instructions' class='mapster-add-submission-shortcode-button thickbox add-new-h2'>Allow User Submissions</a>");
					})
        </script>
				<div id="mapster-user-submission-instructions" style="display:none;">
		      <div>
		        <p>Add a shortcode to any page to allow users to create geographies and submit them into your site.</p>
		        <p>Features can be restricted to any or all geographic types, given an optional category for easy addition later, and marked as drafts or published immediately.</p>
						<hr />
						<table style="margin-bottom: 10px;">
							...
							<tr>
								<td><input id="mapster-submission-longitude" class="mapster-submission-value" type="number" value="0" placeholder="Longitude" /></td>
								<td><input id="mapster-submission-latitude" class="mapster-submission-value" type="number" value="0" placeholder="Latitude" /></td>
								<td><input id="mapster-submission-zoom" class="mapster-submission-value" type="number" value="1" placeholder="Zoom" /></td>
							</tr>
						</table>
						<div style="margin-bottom: 10px;">
							<label style="margin-right: 10px;">Points</label>
							<input id="mapster-submission-allow_points" class="mapster-submission-value" checked="checked" type="checkbox" />
							<label style="margin-right: 10px;">Lines</label>
							<input id="mapster-submission-allow_lines" class="mapster-submission-value" checked="checked" type="checkbox" />
							<label style="margin-right: 10px;">Polygons</label>
							<input id="mapster-submission-allow_polygons" class="mapster-submission-value" checked="checked" type="checkbox" />
							<label style="margin-right: 10px;">Geocoder</label>
							<input id="mapster-submission-include_geocoder" class="mapster-submission-value" checked="checked" type="checkbox" />
							<label style="margin-right: 10px;">Publish</label>
							<input id="mapster-submission-publish" class="mapster-submission-value" type="checkbox" />
						</div>
						<div>
							<label style="margin-right: 10px;">Save as Category</label>
							<select id="mapster-submission-mark_as_category" class="mapster-submission-value" >
		        ...
		        <div style="margin-top: 10px; background: #ccc; font-family: 'Courier New', monospace; color: #333; padding: 10px;">
		          [mapster_wp_map_submit
		            longitude="<span id="mapster-submission-longitude-val">0</span>"
								latitude="<span id="mapster-submission-latitude-val">0</span>"
		            zoom="<span id="mapster-submission-zoom-val">1</span>"
		            allow_points="<span id="mapster-submission-allow_points-val">true</span>"
		            allow_lines="<span id="mapster-submission-allow_lines-val">true</span>"
		            allow_polygons="<span id="mapster-submission-allow_polygons-val">true</span>"
		            mark_as_category="<span id="mapster-submission-mark_as_category-val"></span>"
		            include_geocoder="<span id="mapster-submission-include_geocoder-val">true</span>"
		            publish="<span id="mapster-submission-publish-val">false</span>"]
		        </div>
		        <p>Modify the shortcode as needed and place it wherever you need on your site, and a button will appear that opens a modal for users to submit locations.</p>
		      </div>
				</div>
    	<?php
		} else {
	...
	public function add_mapster_wp_map_metabox() {
		add_meta_box( 'mapster-wp-maps-preview', 'Map Preview', 'my_meta_box_callback',
	...
	/**
	 * If a plugin is enabled, then return certain fields
	 *
	 * @since    1.0.0
	 */
	function mapster_wp_maps_determine_field_visibility( $field ) {
		$return_field = $field;

		$mwm_mapbox_link_fields = array('secret_access_token');

		$plugin_addons = array('mwm-mapbox-link/mwm-mapbox-link.php');
		$plugin_fields = array($mwm_mapbox_link_fields);

		foreach($plugin_addons as $key=>$plugin_name) {
			if(in_array($field['_name'], $plugin_fields[$key])) {
