<?php
  class Mapster_Wordpress_Maps_Admin_API {
    ...
    public function mapster_wp_maps_frontend_submission() {
        register_rest_route('mapster-wp-maps', 'add-feature', array(
            'methods'   => 'POST',
            'callback'  => 'mapster_wp_maps_add_feature',
            'permission_callback' => function(){
                return true; // open to public
            },
        ));

        function mapster_wp_maps_add_feature($request) {

          $body = $request->get_body();
          $decoded_body = json_decode($body);

          $feature = $decoded_body->feature;
          $post_title = $decoded_body->post_title;
          $post_description = $decoded_body->post_description;
          $category_id = $decoded_body->category_id;
          $publish = $decoded_body->publish;
          ...
          if($feature->geometry->type == 'Point') {
            $new_shape = wp_insert_post(array(
              ...
              'post_status' => $publish == 'false' ? 'draft' : 'publish',
    ...
    foreach($field_object_data as $key=>$data) {
      ...
      if($key == 'popup') {
        ...
        if($single_feature_data['data'][$key]['featured_image']) {
          ...
          $newImageData['url'] = $single_feature_data['data'][$key]['featured_image']['sizes']['medium'];
      ...
      if($key == 'images') {
        foreach($field_object_data[$key]['value'] as $image) {
          $thisAttachment = wp_get_attachment_image_src($image['id'], 'medium');
